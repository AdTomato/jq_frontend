...
import {
  Table, Pagination, Button, Icon, Tooltip
} from 'h3-antd-vue';
...
export default class QueryInstance extends mixins(WorkItemMixin) {
  ...
  reloadMessage(event:any) {
    ...
    if (event.data.indexOf('/workflow-center/query-instance') !== -1 || event.data.indexOf('%2Fworkflow-center%2Fquery-instance') !== -1) {
      this.listInstances();
    }
  }
  ...
  */
  onSearch(params:workflowCenterParams.ListInstancesParams) {
    let type:string =  '';
    if (this.curKey === 'PROCESSING' || this.curKey === '' || this.curKey === 'COMPLETED' || this.curKey === 'EXCEPTION') {
      type = 'processing';
    } else {
      type = 'canceled';
    }
  ...
  i18nHandle(item:any){
    switch (item.state) {
      ...
      case WorkflowCenterTypes.WorkflowInstanceState.PROCESSING:
        item.stateTxt = this.$t('cloudpivot.flowCenter.pc.workflowState.processing');
        break;
      case WorkflowCenterTypes.WorkflowInstanceState.COMPLETED:
        item.stateTxt = this.$t('cloudpivot.flowCenter.pc.workflowState.completed');
        break;
      case WorkflowCenterTypes.WorkflowInstanceState.EXCEPTION:
        item.stateTxt = this.$t('cloudpivot.flowCenter.pc.workflowState.exception');
        break;
      case WorkflowCenterTypes.WorkflowInstanceState.CANCELED:
        item.stateTxt = this.$t('cloudpivot.flowCenter.pc.workflowState.canceled');
        break;
    }
    return item;
  }
