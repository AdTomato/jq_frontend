...
export default class WorkflowItemMixin extends Vue {
  ...
  */ 
  ...
  tableScrollerHandler(){
    this.$nextTick(() => {
      if (this.isTrident) {
        (document.querySelector('div.ant-table-body') as any).style.overflow = 'scroll';
        (document.querySelector('div.ant-table-body') as any).style.maxHeight = this.scrollY + 'px';
        
        // cssText('overflow: scroll; max-height:' + this.scrollY+ 'px')
        return;
      }
