...
export default class FormList extends Vue {
    ...
    if (res.data && Array.isArray(res.data.queryConditions) && res.data.queryConditions.length) {
      res.data.queryConditions.forEach((condition: any) => {
        ...
        if (condition.propertyType === 3 && condition.dateType) {
          const date = this.setDateByDateType(condition.dateType);
  ...
  setDateByDateType(type:any) {
    ...
    switch (type) {
      /* 最近一周 */
      case 1:
        start = GetDateHandle.getLatelyWeek();
        end = GetDateHandle.getToday();
        break;
      /* 最近一月 */
      case 2:
        start = GetDateHandle.getLatelyMonth();
        end = GetDateHandle.getToday();
        break;
      /* 最近一季度 */
      case 3:
        start = GetDateHandle.getLatelyQuarter();
        end = GetDateHandle.getToday();
        break;
      /* 最近一年 */
      case 4:
        start = GetDateHandle.getLatelyYear();
        end = GetDateHandle.getToday();
        break;
      /* 本周 */
      case 5:
