...
export default class AppList extends Vue {
  ...
  mounted() {
    ...
    document.title = `晶奇-${this.applicationPageTitle}` || '晶奇';
  ...
  */
  ...
  async getListConfigData() {
    ...
    if (res.data.length === 0) return this.$message.error(res.errmsg, 3);
      ...
      if (Array.isArray(res.data.queryConditions)) {
        res.data.queryConditions.forEach((condition: any) => {
          ...
          if (condition.propertyType === 3 && condition.dateType) {
            const date = Helper.setDateByDateType(condition.dateType);
            condition.startValue = date[0] ? moment(date[0], 'YYYY-MM-DD') : date[0];
            condition.endValue = date[1] ? moment(date[1], 'YYYY-MM-DD') : date[1];
          }
  ...
  */
  async deleteListItems() {
    ...
    if (res.errcode === 0) {
      ...
      vm.$message.error(this.$t('cloudpivot.list.pc.DeleteFailed'), 3);
    }
  }
  ...
  */
  async goForm(column: any, rowIndex: number) {
    ...
    // 否则返回一个字符串
      ...
      // console.log('eeeeeeeeeeeeeeeeeeeee', url)
