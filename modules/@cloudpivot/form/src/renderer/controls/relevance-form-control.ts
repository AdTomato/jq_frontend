  ...
  async setDefaultValue(schemaCode: string, queryCode: string) {
    ...
    if (RelevanceFormControl.service.queryDefaultValue) {
      ...
      if (res.data.length > 0) {
        ...
        // 关联表单 显示它默认字段.
        if (!~Object.keys(item).indexOf(this.control.options.displayField)) {
  ...
  async setLinkageValue() {
    ...
    if (_ctrl.value && _ctrl.value.id) {
      if (field) {
        ...
        // const d = keyGroup.find(res => _ctrl.value[res] === undefined);
        const d = keyGroup.find(res => !(res in _ctrl.value));

        const options = this.control.options;

        if (d || !(field in _ctrl.value)) {
          let value: any = null;
          const dataitems = await RelevanceFormControl.service.dataitemsOf(
            options.schemaCode
          );
          console.log(options.queryCode, options.schemaCode, _ctrl.value.id);
          ...
          if (errcode !== 0) {
            ...
            if (value) {
              const formControls = this.getFormControls();
              let item = await ReverseQueryService.convertForMappings(
                value,
                dataitems,
                _ctrl.options.mappings,
                formControls
