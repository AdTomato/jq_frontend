...
export abstract class FormBuilderHelper {
  ...
  static setFormatter(type: forms.FormControlType, options: any, control: RendererFormControl) {

    switch (type) {
      case forms.FormControlType.Date:
        options.formatter = (c: forms.DateControl) => dateFormatter(c.value, control.options.format)
        break;

      case forms.FormControlType.Number:
        options.formatter = (c: forms.NumberControl) => numberFormatter(control.options.format, c.value)
        break;
      ...
      case forms.FormControlType.Select:
        options.formatter = (c: forms.SelectControl) => {
          ...
          if (Array.isArray(val)) {
            return val.map(x => typeof x === 'string' ? x : (x.name || '')).join(',')
          }
          return '';
        }
        break;
