...
export abstract class StaffSelectorBase extends Vue {
  ...
  async mappingOperate(item: any) {
    let self: any = this;
    let mappings = this.controlOpts ? this.controlOpts.mappings : '';
    if (mappings) {
      // 获取所有映射的 字段
  ...
  async getUsers(deptId: string, filterType?: string) {
    const users = await StaffSelectorControl.service.getUsersBy(
      deptId,
      this.roleId,
      filterType
    );
  ...
  onSearch(name: string) {
    ...
    StaffSelectorControl.service
      .search(name, this.rootIds, this.roleId, this.filterType)
      .then(data => {
        const list: any[] = [];
