  ...
  if (kind.includes('column')) {
    ...
    if (typeof codeObj[code] === 'object') {
      ...
      if (codeObj[code].length && codeObj[code][0].name) {
        ...
        if (mappings) {
          if (Array.isArray(codeObj[code][mappings])) {
            // @ts-ignore
            itemOne[str].innerTxt = codeObj[code][mappings][0].name || "";
          }
          else {
            if (mappings.includes("Address")) {
              const add = JSON.parse(codeObj[code][mappings]);
              // @ts-ignore
              itemOne[str].innerTxt = add.address || "";
            }
            else {
...
function dateAndNumberFilter(code: string, forDataProperty: object, itemOne: object, str: string, kind: string, codeObj: object, eleType: string) {
  ...
  if (forDataProperty.bizSheet.publishedAttributesJson) {
    // @ts-ignore
    dataProperties = JSON.parse(forDataProperty.bizSheet.publishedAttributesJson);
      ...
      for (let propItem in dataProperties) {
        if (code === propItem) {
          if (kind.includes('date')) {
            ...
            eleType.includes('column')
              // @ts-ignore
              ? itemOne[str].innerTxt = formatter(codeObj[code], dataProperties[propItem].options.format)
              // @ts-ignore
              : itemOne.innerTxt = formatter(codeObj[code], dataProperties[propItem].options.format);
            return;
          }
