    ...
    <div v-else-if="list.length" class="data-permission__main">
      ...
      <div class="data-permission__table">
        ...
        >
          ...
          <span slot="departmentTitle" class="resize">组织</span>
          ...
          <span slot="departments" slot-scope="text,record">{{ record.departmentNames }}</span>
          <a-switch
            slot="enable"
            slot-scope="text,record"
...
export default class DataPermission extends Vue {
  ...
  onEnableChange(enabled: any, item: any) {
    const switchParams: any = {
      ...item,
      enabled,
      departments: JSON.stringify(item.departments),
      ...
      if (res.errcode === 0) {
        this.list = res.data.map((item: any, idx: number) => {
          ...
          const resOther: any = {
            ...
            departmentNames: departments.map((department: any) => department.name).join('，')
          };
          return resOther;
        });
