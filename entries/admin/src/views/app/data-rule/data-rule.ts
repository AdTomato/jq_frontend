...
export default class DataRuleHandler extends Vue {
  ...
  triggerType = DataRuleType.TriggerType.Main;

  triggerSchemaCodeIsGroup:boolean = false;
  ...
  get isMain() {
    return (this.triggerType === DataRuleType.TriggerType.Main);
  }
  ...
  selectTriggerObj(val:any, vNode:any) {
    this.triggerType = DataRuleType.TriggerType.Main;
  ...
  setCurFieldsList(propertyType:number, code:string) {
    ...
    if (propertyType === -1) { // 模型
      ...
      let sheetCode = code;
      if (idx > 0) {
        sheetCode = code.slice(idx + 2);
      };
      this.triggerType = DataRuleType.TriggerType.MainAndSheet;
      // this.isSubSchema = true;
      const subSchemaTem = dataItemsCopy.find((item:any) => item.code === sheetCode);
      const subSchemaMain = dataItemsCopy.filter(res => res.propertyType !== 8 && res.published);
      if (subSchemaTem.subSchema) {
        list = subSchemaTem.subSchema.properties.filter((item:any) => item.published).map(item => {
          return {
            ...item,
            copyCode: item.code,
            code: `${sheetCode}.${item.code}`,
          }
        });
        debugger;
        list = [...subSchemaMain, ...list];
      }
    } else { // 子表
      // debugger;
      // this.isSubSchema = true;
      this.triggerType = DataRuleType.TriggerType.SubSheet;
  ...
  async getTriggerObjLists() {
    ...
    if (res.errcode === 0) {
      ...
      const mainAndSheet = subSchemaLists.map(res2 => {
        return {
          ...res2,
          copyCode: res2.code,
          code: `${bizSchemaCode}&&${res2.code}`,
          propertyType: -2
        }
      });
      // 再拼接上当前模型
      ...
      this.triggerObjLists = [...subSchemaLists, ...mainAndSheet] ;
    }
  }

  setTriggerObjOptionVal(item:any) {
    switch(item.propertyType) {
      case -1:
        return `模型-${item.name}[${item.code}]`;
      case 8: 
        return `子表-${item.name}[${item.code}]`;
      case -2:
        return `主表+子表-${item.name}[${item.copyCode}]`;
    }
  }
  ...
  */
  ...
  setCurrentDataItems(type:DataItemType, code: string, rowData:any, Datatype: DataRuleType.DataType) {
    ...
    // 关联表单：
    // 1.匹配数据项绑定 relativeCode 一致；
    // 2.如果关联当前模型 主表可选 id 子表可选 parentId；
    if (type === DataItemType.RelevanceForm) {
      ...
      if (ctargetCode && ctargetCode.relativeCode === bizSchemaCode) {
        ...
        if (this.triggerType === DataRuleType.TriggerType.Main) {
          filterCode.push('id');
        } else if (this.triggerType === DataRuleType.TriggerType.SubSheet) {
          filterCode.push('parentId');
        } else {
          filterCode.push(`${this.triggerVal}.parentId`,'id');
        }
        const idData = list.filter((item:any) => filterCode.includes(item.code));
        arr.push(...idData);
      };
    }
    ...
    // 目前模型选择业务对象ID/数据标题/单据状态时，当前模型隐藏子表业务对象ID和子表业务对象parentID

    const filterId = ['id', 'name', 'sequenceStatus'];
    const filterIdAndParentId = [`${this.triggerVal}.parentId`, `${this.triggerVal}.id`, 'id']


    if (filterId.includes(code)) {
      arr = arr.filter(item => {
        return !filterIdAndParentId.includes(item.code);
      })
    };
  ...
  get triggerVal() {
    const idx = this.triggerObj.indexOf('&&');
    if (idx > 0) {
      return this.triggerObj.slice(idx + 2);
    } else {
  ...
  */
  ...
  paramsFormater(type:string) {
    ...
    let triggerSchemaCode = this.triggerObj;

    const idx = triggerSchemaCode.indexOf('&&');

    if (idx > 0) {
      triggerSchemaCode = triggerSchemaCode.slice(idx + 2);
    }
    ...
    if (idx > 0) {
      params.triggerSchemaCodeIsGroup = true;
    }
  ...
  async postData (type: DataRuleType.OperationMode) {
    ...
    let res:any =  null;
    if (isAdd) {
      res =  await DataRuleApi.create(params);
    } else {
      res =  await DataRuleApi.update(params);
    }
    if (res.errcode === 0) {
  ...
  async dataBackWriting() {
    ...
    // 数据回写
      const {
        ...
        dataConditionJoinType, dataConditionJson, targetTableCode, ruleScopeChildJson, ruleActionMainScopeJson, insertConditionJoinType, triggerSchemaCodeIsGroup
      } = this.data;
      ...
      this.triggerObj = triggerSchemaCodeIsGroup ? `${sourceSchemaCode}&&${triggerSchemaCode}` : triggerSchemaCode;
      let ptype:number = 0;

      if (triggerSchemaCodeIsGroup) {
        ptype = -2;
      } else {
        ptype = triggerSchemaCode === sourceSchemaCode ? -1 : 0;
      }
      ...
      if (this.dataConditionJoinType !== 1) {
        ...
        dataConditionData.forEach((item:any, index) => {
