        ...
        if (el.tagName.indexOf('a-') === 0) {
            ...
            for (const attr of el.attrs) {
                ...
                if (idx > -1) {
                    ...
                    if (value && typeof value === 'object' && key !== 'name_i18n' && key !== 'heads') {
                    ...
                    if (attr.name === 'key' || (inSheet && attr.name === 'width')) {
                        ...
                        control[attr.name] = FormElementParseService.convertAttrValue(attr.value);
            ...
            const options = renderer.FormControlOptionsService.buildFor(controlType, opts);
            if (options) {
        ...
        for (const col of el.children) {
            ...
            if (type === schema.FormControlType.Html) {
                ...
                if (type === schema.FormControlType.Sheet) {
                    ...
                    tabs.panels = child.children.filter(c => c.tagName === 'a-tabs-panel')
                        .map<schema.FormTabsPanel>(panelEl => {
                            const panel = FormElementParseService.buildBaseControlOf<schema.FormTabsPanel>(panelEl, tabs);
                            if (!panel) {
