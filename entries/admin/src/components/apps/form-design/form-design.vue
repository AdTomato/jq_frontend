...
export default class FormDesign extends Vue {
  ...
  onControlUpdate(
    ...
    if(parents && parents.length > 0){
      ...
      if(parents[0].type === FormControlType.Tabs){
        ...
        const [ rowIdx, colIdx ] = ControlFactory.findIndexFromGrid(layout, control.key);
  ...
  initLayout() {
    ...
    for (const key in controls) {
      ...
      if (control.type === FormControlType.Sheet) {
        ...
        control.columns = control.columns.filter((val) => val.code !== FormControlType.SystemOther) // 子表过滤 子表排序号 
        if (item && item.properties) {
  ...
  updateForm(isPublish: boolean, msg: string) {
    ...
    if (addCodes.length) {
      ...
      data.properties = addCodes.map(code => {
        ...
        obj.subSchema.properties = item.properties.filter(p => !p.isSystem).map(p => {
          let ob:any = {
            schemaCode,
            id: p.id,
            code: p.code,
