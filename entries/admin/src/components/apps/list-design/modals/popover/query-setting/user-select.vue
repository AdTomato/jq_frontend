...
export default class SystemText extends Vue {
  ...
  get userDefaultTypes () {
    if (this.sysUserArr.includes(this.backData.propertyCode) || this.backData.userOptionType === 1) {
      return UserDefaultType.filter((user:any) => user.type !== 2);
    } else if (this.sysDeptArr.includes(this.backData.propertyCode) || this.backData.userOptionType === 2) {
