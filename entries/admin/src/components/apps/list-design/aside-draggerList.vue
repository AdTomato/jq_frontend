      ...
      <draggable
        ...
        <transition-group>
          ...
          >
            ...
            <span :title="el.propertyName" class="bar-title each-title text-ellipsis">{{ el.name_i18n[$i18n.locale] || el.name }}</span>
...
export default class SideBoxDraggerList extends Vue {
    ...
    new Promise((resolve,reject)=>{
      if (this.draggerlistData.length > 0) {
        if (this.fieldBlock === 3) {
          ...
          // const item = this.draggerlistData[i];

          // let locale = this.$i18n.locale
          // let isChinese  = locale==='zh';
          // let realActionName = isChinese? val.name: name_i18n[locale];


          // if ( !realActionName.trim() ) {
          //   if ( isChinese ) {
          //     val.name = item.name;
          //   }
          //   else {
          //     val.name_i18n = {...item.name_i18n};
          //   }
          // }


          this.draggerlistData[i].name = val.name;
          this.draggerlistData[i].name_i18n = {...val.name_i18n};
      ...
      this.hideLeve2Popoer(params, val, true);
    }).catch((error) => {
      // console.log(error);
  ...
  /* li弹窗-取消 */
  hideLeve2Popoer(i:number, backData, justClose=false) {

    // if ( !justClose && this.fieldBlock===3  ) {
    //   const item = this.draggerlistData[i];
    //   let locale = this.$i18n.locale
    //   let isChinese  = locale==='zh';
    //   let realActionName = isChinese? backData.name: backData.name_i18n[locale];

    //   if ( isChinese ) {
    //     backData.name = item.name;
    //   }
    //   else {
    //     backData.name_i18n = {...item.name_i18n};
    //   }
    // }
