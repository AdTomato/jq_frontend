  ...
  <div class="app-header">
    ...
    <div class="header-content flex-center">
      ...
      <div
        ...
        @mouseleave.stop="mouseLeaveMenu('wf')"
      >
        <template v-if="workflows.length >= 2">
          <div class="content">
            <div
              class="cursor-pointer"
    ...
    switch (type) {
      ...
      case 'wf':
        ...
        if (this.workflows.length === 0) {
          if(this.$route.name === 'workflowEmpty') return;
          this.$router.push({ name: 'workflowEmpty' });
        }
        break;
  ...
  changeHeightLight(val: string) {
    ...
    for (let i = 0; i < this.modules.length; i += 1) {
      if (this.modules[i].name === val) {
        ...
        this.moduleName = ''+this.$t('languages.Apps.'+ this.modules[i].code);
      } else {
