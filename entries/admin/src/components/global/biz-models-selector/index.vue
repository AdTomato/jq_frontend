...
export default class bizModelsSelector extends Vue {
  ...
  onChange(val: string) {
    ...
    if (!val) {
      ...
      if (this.canSelectAll && node.dataRef && node.dataRef.type === 'external') {
  ...
  setDefault() {
    ...
    this.onLoadData(treeNode).then((res: any) => {
      if (res) {
        const theApp = res.filter(app => app.value === this.appCode);
        if (theApp) {
          // debugger
          const theKey = this.getDefaultValue(this.value, theApp[0].children);
          if (theKey) {
            this.result = [theKey];
          }
        }
