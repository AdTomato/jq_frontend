...
export class TemplateHandle {
    ...
    htmlHandle () {
        ...
        let styles: any = this.rootTemplateValidate.styles;
        ...
        let scripts = this.rootTemplateValidate.scripts;
        ...
        let styleLinks: any = document.body.querySelectorAll('style,link');
        ...
        let childNodes: any = document.body.childNodes;
        for(let i=0;i<childNodes.length;i++){
            if(childNodes[i].nodeType == 8 || (childNodes[i].tagName && (childNodes[i].tagName !== 'SCRIPT'  || childNodes[i].id == 'customScript'))){
...
export  class TemplateValidate{
    ...
    private toolbarHandle () :number {
        ...
        if($toolbar){
            ...
            for(let i =0;i<actions.length;i++){
                let text = actions[i].getAttribute('text');
                let code =actions[i].getAttribute('code');
                if(!!text && !!code){
                    data.push({ text, code });
                }else{
                    res = 3;
                ...
                if(tagName === 'a-sheet'){
                 this.sheetHandle(element, templateControl);
                    ...
                    for(let i=0;i<nodes.length;i++){
                        let templateControlOther = this.elementConvertJson(nodes[i]);
  ...
  private sheetHandle(element: any, templateControl: any) {
      ...
      if ($columns && $columns.children.length) {
          let childrens =  Array.prototype.slice.call($columns.children);
          for(let i=0;i<childrens.length;i++){
            columns.push(this.resolveElementAttribute(childrens[i], TemplateBPMType[childrens[i].tagName.toLocaleLowerCase()], templateControl.BPMOptions.key));
          }
      ...
      if ($totals && $totals.length) {
          for(let i=0;i<$totals.length;i++){
            statistics.push(this.resolveElementAttribute($totals[i], TemplateBPMType[$totals[i].tagName.toLocaleLowerCase()], templateControl.BPMOptions.key));
          }
